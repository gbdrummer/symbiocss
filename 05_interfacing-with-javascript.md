...
`id`s have a place in SymbioCSS, but not for styling. `id`s are meant to provide direct access to a specific DOM element. To accomplish this, in CSS they are granted a much higher degree of specificity than class or tag selectors. However, the amount of extra specificity granted is **arbitrary**. As a result, `id`s can cause a lot of specificity problems when combined with other types of selectors. It is much easier to simply avoid using them for styling.
